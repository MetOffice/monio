...
In the following scenarios, the only common parameter is a `atlas::FieldSet` that uses an `atlas::CubedSphereGrid` and an `atlas::Mesh` configured to a `"cubedsphere_dual"`. Passing a `atlas::FieldSet` of a different configuration may cause issues that prevent MONIO from functioning correctly.
